...
class MotionVectorDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir="data", mode="train", codec="mpeg4",
        transforms=None, scales=[1.0, 0.75, 0.5], static_only=False,
    ...
    def __getitem__(self, idx):
        ...
        motion_vectors_upsampled = get_nonzero_vectors(motion_vectors)
        motion_vectors_upsampled = motion_vectors_to_image(motion_vectors_upsampled, (frame.shape[1], frame.shape[0]))
        if self.codec == "mpeg4":
            motion_vectors_dense = motion_vectors_to_grid(motion_vectors, (frame.shape[1], frame.shape[0]))
        elif self.codec == "h264":
            motion_vectors_dense = motion_vectors_to_grid_interpolated(motion_vectors, (frame.shape[1], frame.shape[0]))

        motion_vectors_upsampled = torch.from_numpy(motion_vectors_upsampled).float()
        motion_vectors_dense = torch.from_numpy(motion_vectors_dense).float()
        ...
        # scale down boxes by factor 16 for use with dense motion vectors
        boxes = boxes / 16.0
        boxes_prev = boxes_prev / 16.0

        sample = {
            "motion_vectors_upsampled": motion_vectors_upsampled,
            "motion_vectors_dense": motion_vectors_dense,
        ...
        sample["motion_vectors_upsampled"] = sample["motion_vectors_upsampled"][..., [2, 1, 0]]
        sample["motion_vectors_dense"] = sample["motion_vectors_dense"][..., [2, 1, 0]]

        # swap motion vector axes so that shape is (C, H, W) instead of (H, W, C)
        sample["motion_vectors_upsampled"] = sample["motion_vectors_upsampled"].permute(2, 0, 1)
        sample["motion_vectors_dense"] = sample["motion_vectors_dense"].permute(2, 0, 1)
...
if __name__ == "__main__":
    ...
    transforms = {
        "train": torchvision.transforms.Compose([
            ...
            #StandardizeMotionVectors(mean=Stats.motion_vectors["mean"], std=Stats.motion_vectors["std"], items=["motion_vectors_upsampled", "motion_vectors_dense"]),
    ...
    datasets = {x: MotionVectorDataset(root_dir='data', transforms=transforms[x],
        codec=codec, scales=scales, static_only=static_only,
    ...
    for batch_idx in range(batch_size):
        ...
        cv2.namedWindow("motion_vectors-upsampled-{}".format(batch_idx), cv2.WINDOW_NORMAL)
        cv2.resizeWindow("motion_vectors-upsampled-{}".format(batch_idx), 640, 360)
        cv2.namedWindow("motion_vectors-dense-{}".format(batch_idx), cv2.WINDOW_NORMAL)
        cv2.resizeWindow("motion_vectors-dense-{}".format(batch_idx), 640, 360)
    ...
    for step, sample in enumerate(dataloaders["train"]):
        ...
        for batch_idx in range(batch_size):
            ...
            motion_vectors_upsampled = sample["motion_vectors_upsampled"][batch_idx]
            motion_vectors_dense = sample["motion_vectors_dense"][batch_idx]
            boxes_prev = sample["boxes_prev"][batch_idx]
            boxes = sample["boxes"][batch_idx]
            velocities = sample["velocities"][batch_idx]

            motion_vectors_upsampled = motion_vectors_upsampled.permute(1, 2, 0)
            motion_vectors_upsampled = motion_vectors_upsampled[..., [2, 1, 0]]
            motion_vectors_upsampled = motion_vectors_upsampled.numpy()
            motion_vectors_upsampled = (motion_vectors_upsampled - np.min(motion_vectors_upsampled)) / (np.max(motion_vectors_upsampled) - np.min(motion_vectors_upsampled))

            motion_vectors_dense = motion_vectors_dense.permute(1, 2, 0)
            motion_vectors_dense = motion_vectors_dense[..., [2, 1, 0]]
            motion_vectors_dense = motion_vectors_dense.numpy()
            motion_vectors_dense = (motion_vectors_dense - np.min(motion_vectors_dense)) / (np.max(motion_vectors_dense) - np.min(motion_vectors_dense))

            print("step: {}, motion_vectors_upsampled: {}, motion_vectors_dense: {}".format(step, motion_vectors_upsampled.shape, motion_vectors_dense.shape))

            cv2.imshow("frame-{}".format(batch_idx), frame)
            cv2.imshow("motion_vectors-upsampled-{}".format(batch_idx), motion_vectors_upsampled)
            cv2.imshow("motion_vectors-dense-{}".format(batch_idx), motion_vectors_dense)
