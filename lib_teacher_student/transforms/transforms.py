...
class StandardizeMotionVectors:
    ...
    Args:
        items (`list` of `str`): A list of key(s) which are used to select the
            motion vectors from the sample provided during call of this
            transform. E.g. can be ["motion_vectors_dense",
            "motion_vectors_upsampled"].
    ...
    """
    def __init__(self, mean, std, items=["motion_vectors"]):
    ...
    def __call__(self, sample):
        ...
        for item in self.items:
            motion_vectors = sample[item].clone()
            motion_vectors = (motion_vectors - self.mean) / self.std
            sample_transformed[item] = motion_vectors
