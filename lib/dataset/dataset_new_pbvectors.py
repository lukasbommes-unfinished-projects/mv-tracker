...
class MotionVectorDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir="data", mode="train", codec="mpeg4",
        ...
        vector_type="p", static_only=False, exclude_keyframes=True, visu=False,
        debug=False):
    ...
    def process_motion_vectors_(self, motion_vectors, frame):
        ...
        return motion_vectors, motion_vectors_for_visu
    ...
    def __getitem__(self, idx):
        ...
        if self.vector_type == "p" or self.vector_type == "p+b":
            ...
            motion_vectors_p, motion_vectors_p_for_visu = self.process_motion_vectors_(motion_vectors_p, frame)

        if self.vector_type == "b" or self.vector_type == "p+b":
            motion_vectors_b = get_vectors_by_source(motion_vectors, "future")  # get only b vectors
            motion_vectors_b, motion_vectors_b_for_visu = self.process_motion_vectors_(motion_vectors_b, frame)

        if self.vector_type == "p":
            motion_vectors = motion_vectors_p
        elif self.vector_type == "b":
            motion_vectors = motion_vectors_b
        elif self.vector_type == "p+b":
            motion_vectors = [motion_vectors_p, motion_vectors_b]
        ...
        if self.vector_type == "p" or self.vector_type == "b":
