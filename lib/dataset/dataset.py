...
from lib.dataset.loaders import load_groundtruth, load_detections
...
class MotionVectorDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir, mode, batch_size, codec="mpeg4", pad_num_boxes=54, mot17_detector="FRCNN", visu=False, debug=False):
        ...
        self.mot17_detector = mot17_detector  # load data generated by which detector

        self.det_boxes_prev = None
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if not self.is_open[self.current_seq_id]:
                ...
                # open detections depending on which detector should be used
                sequence = self.sequences[self.mode][self.current_seq_id]
                if sequence.split("/")[0] == "MOT17":
                    sequence_name = sequence.split("/")[-1]
                    current_detector = sequence_name.split("-")[-1]  # e.g. FRCNN
                    detections_path = sequence.replace(current_detector, self.mot17_detector)
                else:
                    detections_path = sequence
                detections_file = os.path.join(self.root_dir, detections_path, "det/det.txt")
                if self.DEBUG:
                    print("Detection files of {} detector loaded".format(self.mot17_detector))
                ...
                while True:
                    ...
                    if self.last_gt_was_none:
                        ...
                        self.det_boxes_prev = self.detections[self.current_frame_idx]
                        if self.DEBUG:
            ...
            det_boxes_prev_padded = torch.zeros(self.pad_num_boxes, 4).float()
...
if __name__ == "__main__":
    ...
    datasets = {x: MotionVectorDataset(root_dir='data', batch_size=batch_size,
        codec=codec, mot17_detector="SDP", visu=True, debug=True,
