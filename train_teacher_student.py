...
from lib_teacher_student.models.pnet_teacher_student import PropagationNetwork
from lib_teacher_student.dataset.dataset_teacher_student import MotionVectorDataset
from lib_teacher_student.dataset.stats import StatsMpeg4DenseStaticSinglescale as Stats
from lib_teacher_student.transforms.transforms import StandardizeMotionVectors, \
    StandardizeVelocities, RandomFlip, RandomMotionChange
from lib_teacher_student.dataset.velocities import box_from_velocities
from lib_teacher_student.utils import compute_mean_iou, weight_checksum, count_params
...
def train(model_teacher, model_student, optimizer, scheduler, batch_size, num_epochs,
    ...
    criterion_features = nn.MSE(reduction='mean')

    best_loss = 99999.0
    best_mean_iou = 0.0
    iterations = {"train": 0, "val": 0}

    logger.info("Weight sum before training: {}".format(weight_checksum(model_student)))
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ["train", "val"]:
            ...
            for step, sample in enumerate(dataloaders[phase]):
                ...
                motion_vectors_upsampled = sample["motion_vectors_upsampled"]
                motion_vectors_dense = sample["motion_vectors_dense"]
                ...
                with torch.set_grad_enabled(phase == "train"):
                    ...
                    print("Shape motion_vectors_upsampled:", motion_vectors_upsampled.shape)
                    print("Shape motion_vectors_dense:", motion_vectors_dense.shape)

                    _, features_teacher = model_teacher(motion_vectors_upsampled, boxes_prev*16.0)
                    velocities_pred, features_student = model_student(motion_vectors_dense, boxes_prev)

                    # compute velocities loss
                    loss_velocitiy = criterion_velocity(velocities_pred, velocities)
                    loss_features = criterion_features(features_student, features_teacher)

                    print("loss_velocitiy:", loss_velocitiy)
                    print("loss_features:", loss_features)

                    loss = loss_velocitiy + loss_features

                    if phase == "train":
                        if write_tensorboard_log:
                            params_before_update = [p.detach().clone() for p in model_student.parameters()]
                        ...
                        if (step + 1) % batch_size == 0:
                            ...
                            if write_tensorboard_log:
                                params_after_update = [p.detach().clone() for p in model_student.parameters()]
        ...
        if write_tensorboard_log:
            log_weights(model_student, epoch, writer)

    time_elapsed = time.time() - tstart
    logger.info('Training complete in {:.0f}m {:.0f}s'.format(time_elapsed // 60, time_elapsed % 60))
    logger.info('Lowest validation loss: {}'.format(best_loss))

    logger.info("Weight sum after training: {}".format(weight_checksum(model_student)))
...
if __name__ == "__main__":
    ...
    datasets = {x: MotionVectorDataset(root_dir=args.root_dir, transforms=transforms[x],
        codec=args.codec, scales=args.scales, static_only=args.static_only,
    ...
    model_teacher = PropagationNetwork()
    model_student = PropagationNetwork()

    model_teacher = model_teacher.to(device)
    model_student = model_student.to(device)

    optimizer = optim.Adam(model_student.parameters(), lr=args.learning_rate, betas=(0.9, 0.999),
    ...
    train(model_teacher, model_student, optimizer, scheduler=scheduler,
        batch_size=args.batch_size, num_epochs=args.num_epochs,
        write_tensorboard_log=write_tensorboard_log, save_model=save_model,
        save_model_every_epoch=save_model_every_epoch, outdir=outdir, logger=logger)
